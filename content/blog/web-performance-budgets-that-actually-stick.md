...
- A budget like JavaScript under 200 kilobytes is forgettable.
- Synthetic tests on fiber connections lie.
- Images are almost always the biggest source of bloat.
- Performance budgets without CI enforcement quietly degrade.
- When the team ships a performance win, celebrate it the same way you celebrate a feature launch.
...
A budget like JavaScript under 200 kilobytes is forgettable. A budget like the product page must be interactive within two seconds on a mid-range Android is a story the team can rally around.
...
- Strip metadata from every image before upload

## Block Regressions in CI

Performance budgets without CI enforcement quietly degrade. A check that fails the pull request when the bundle grows beyond budget is the only way to keep the line over months and years.
...
When the team ships a performance win, celebrate it the same way you celebrate a feature launch. Performance is a feature. Treat it like one and the team will start to compete for the next win.
